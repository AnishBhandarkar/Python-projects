...
lblTitle = Label(Tops, font=('arial', 60, 'bold'), text="          Restaurant Billing Software        ", bd=21, bg='green2',
                 fg='navy', justify=CENTER)
lblTitle.grid(row=0, column=0)

RecieptCal_F = Frame(root, bg='Powder Blue', bd=7, relief=RIDGE)
...
def CostofItem():
        ...
        PriceofMeals = (Item9 * 85) + (Item10 * 60) + (Item11 * 75) + (Item12 * 100) + (Item13 * 250) + (Item14 * 150) + (Item15 * 120) +(Item16 * 140) 

        DrinksPrice = "₹", str('%.2f'%(PriceofDrinks))
        MealsPrrice = "₹", str('%.2f'%(PriceofMeals))
        CostofMeals.set(MealsPrrice)
...
def welcome_bill():
    ...
    txtReceipt.insert(END, '\t\tRestaurant  Receipt\n')
    txtReceipt.insert(END, '         ********************************************************************\n')
...
def Receipt():
        ...
        txtReceipt.insert(END, "\n******************************************************************************\n")
        txtReceipt.insert(END, '\t\tThank you for your visit\n' + '\t\t        Have a nice day')
...
lblCostOfDrinks = Label(Cost_F, font=('arial', 14, 'bold'), text="   Cost of Drinks\t\t  ", bd=21, bg='Cyan', fg='black')
...
txtCostOfMeals = Entry(Cost_F, bg="white", bd = 7, insertwidth=2, font=('arial', 14, 'bold'), justify=RIGHT, textvariable=CostofMeals)
txtCostOfMeals.grid(row=1, column=1)

lblServicecharge = Label(Cost_F, font=('arial', 14, 'bold'), text="  Cost of Service    ", bd=21, bg='Cyan', fg='black')
...
lblSubTotal.grid(row=0, column=2, sticky=W)
...
w = Scrollbar(Reciept_F, orient=VERTICAL)
txtReceipt = Text(Reciept_F, width = 52, height = 26, bg="pink1", bd = 4, fg='navy', font=('arial', 12, 'bold'), yscrollcommand=w.set)

w.pack(side=RIGHT, fill=Y)
w.config(command=txtReceipt.yview)

txtReceipt.pack()

#------------------------------------------------------Receipt Buttons-----------------------------------------------------------------

btnTotal = Button(Buttons_F, padx=16, pady=1, bd=7, fg="black", font=('arial', 19, 'bold'), width=5, text="Total", 
           bg='deep sky blue', command=CostofItem).grid(row=0, column=0)
btnReceipt = Button(Buttons_F, padx=16, pady=1, bd=7, fg="black", font=('arial', 19, 'bold'), width=5, text="Receipt", 
           bg='deep sky blue', command=Receipt).grid(row=0, column=1)
btnReset = Button(Buttons_F, padx=16, pady=1, bd=7, fg="black", font=('arial', 19, 'bold'), width=5, text="Reset", 
           bg='deep sky blue', command=Reset).grid(row=0, column=2)
btnExit = Button(Buttons_F, padx=16, pady=1, bd=7, fg="black", font=('arial', 19, 'bold'), width=5, text="Exit", 
...
helpmenu.add_command(label="1.Mark the item which you need.")
helpmenu.add_command(label="2.Specify the quantity.")
helpmenu.add_command(label="3.Click on Total.")
helpmenu.add_command(label="4.Click on Receipt to generate bill.")
helpmenu.add_command(label="5.Click on Reset to reset everything.")
helpmenu.add_command(label="6.Click Exit to close the system.")
